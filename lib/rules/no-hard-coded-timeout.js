...
module.exports = {
  ...
  create(context) {
      ...
      context.report({
        ...
        fix(fixer) {
          ...
          if (callbackNode.type === 'FunctionExpression' ||
            ...
            // Not a function (e.g., `setTimeout(resolve, 3000)`) — cannot safely
            // produce a meaningful waitFor callback, so skip the autofix.
            return null;
          }
    ...
    function checkPromiseTimeout(node) {
      ...
      // No autofix here — the inner setTimeout is already fixed by checkSetTimeout.
      // Providing a Promise-level fix would conflict with the setTimeout fix or
      // produce a meaningless placeholder like `waitFor(() => ..., { timeout })`.
      context.report({
        node,
