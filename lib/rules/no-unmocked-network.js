...
const API_VARIABLE_PATTERNS = {
  // Matches 'api' but not 'apiKey', 'apis' (common false positives)
  api: /\bapi(?!Key|s\b)/i,

  // Matches 'endpoint'
  endpoint: /\bendpoint/i,

  // Matches 'url' but not 'urls' (plural form)
  url: /\burl(?!s\b)/i,

  // Matches 'host' but not 'hostname' (system property)
  host: /\bhost(?!name\b)/i,

  // Matches 'domain' but not 'domains' (plural)
  domain: /\bdomain(?!s\b)/i
};
      ...
      CallExpression(node) {
        ...
        if (mockModules.includes('fetch') &&
          ...
          if (urlArg && urlArg.type === 'Literal' && typeof urlArg.value === 'string') {
            ...
            // Special case: always flag simple 'url' identifier as it's very commonly used for URLs
            if (urlArg.type === 'Identifier' && text === 'url') {
              context.report({
                node,
                messageId: 'mockNetwork',
                data: { method: 'fetch' }
