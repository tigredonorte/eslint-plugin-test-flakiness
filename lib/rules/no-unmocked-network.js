...
const { isTestFile, isInMockContext, isDataUrl, getFilename } = require('../utils/helpers');
      ...
      CallExpression(node) {
        ...
        if (callee.type === 'MemberExpression') {
          ...
          if ((obj.name === '$' || obj.name === 'jQuery') &&
              ['ajax', 'get', 'post', 'put', 'delete', 'patch', 'getJSON', 'load'].includes(callee.property.name)) {
          ...
          // Libraries that should always be flagged (dedicated HTTP clients)
          const alwaysFlagLibraries = ['request', 'superagent', 'got', 'node-fetch'];
          // Libraries that should only be flagged if in mockModules
          const conditionalLibraries = ['http', 'https'];
          ...
          const isAlwaysFlag = alwaysFlagLibraries.some(lib => objText.includes(lib));
          const isConditionalFlag = conditionalLibraries.some(lib => objText.includes(lib) && mockModules.includes(lib));

          if ((isAlwaysFlag || isConditionalFlag) && httpMethods.includes(method)) {
            context.report({
              node,
              messageId: 'mockNetwork',
