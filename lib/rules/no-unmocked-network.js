      ...
      CallExpression(node) {
        ...
        if (mockModules.includes('fetch') &&
          ...
          if (urlArg && urlArg.type === 'Literal' && typeof urlArg.value === 'string') {
            ...
            // Match variable names that are likely to be network endpoints, but avoid common false positives
            if (/\b(api(?!Key|s\b)|endpoint|url(?!s\b)|host(?!name\b)|domain(?!s\b))\b/i.test(text)) {
              context.report({
                node,
                messageId: 'mockNetwork',
                data: { method: 'fetch' }
