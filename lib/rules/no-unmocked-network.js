...
const API_VARIABLE_PATTERNS = {
  ...
  // Matches 'host' but not 'hostname', 'hosts' or compound forms like 'hostConfig'
  host: /\bhost(?!s\b|name\b|[a-z])/i,
      ...
      CallExpression(node) {
        ...
        if (mockModules.includes('fetch') &&
          ...
          if (urlArg && urlArg.type === 'Literal' && typeof urlArg.value === 'string') {
            ...
            if (COMBINED_API_PATTERN.test(text)) {
              context.report({
                node,
                messageId: 'mockNetwork',
                data: { method: 'fetch' }
        ...
        if (callee.type === 'MemberExpression') {
          ...
          const isAlwaysFlag = ALWAYS_FLAG_LIBRARIES.some(lib => new RegExp(`\\b${lib}\\b`).test(objText));
          const isConditionalFlag = CONDITIONAL_LIBRARIES.some(lib => new RegExp(`\\b${lib}\\b`).test(objText) && mockModules.includes(lib));

          if ((isAlwaysFlag || isConditionalFlag) && HTTP_METHODS.includes(method)) {
            // Find which library matched for accurate reporting
            const matchedLib = ALWAYS_FLAG_LIBRARIES.find(lib => new RegExp(`\\b${lib}\\b`).test(objText)) ||
                               CONDITIONAL_LIBRARIES.find(lib => new RegExp(`\\b${lib}\\b`).test(objText));
            context.report({
              node,
              messageId: 'mockNetwork',
              data: { method: matchedLib || 'http' }
            });
            return;
          }
        }
