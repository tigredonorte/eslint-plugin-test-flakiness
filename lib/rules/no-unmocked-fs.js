...
// Setup hook names for O(1) lookup performance
const SETUP_HOOKS = new Set([
  'beforeAll', 'beforeEach', 'afterAll', 'afterEach',
  'before', 'after', 'setup', 'teardown'
]);

// Child process methods that might touch the filesystem
const CHILD_PROCESS_METHODS = new Set(['exec', 'execSync', 'spawn', 'spawnSync']);

// FS methods array for specific checks
const FS_FILE_METHODS = ['readFile', 'readFileSync', 'writeFile', 'writeFileSync'];
...
module.exports = {
  ...
  create(context) {
    ...
    const allowedModulesSet = new Set(options.allowedModules || []); // Using Set for O(1) lookup
    const mockModules = options.mockModules || ['fs', 'fs/promises', 'node:fs'];

    // Cache source text for performance
    let sourceTextCache = null;
    function getSourceText() {
      if (sourceTextCache === null) {
        sourceTextCache = context.getSourceCode().getText();
      }
    ...
    function isInSetupHook(node) {
      ...
      while (parent) {
        if (parent.type === 'CallExpression' && parent.callee) {
          ...
          if (calleeName && SETUP_HOOKS.has(calleeName)) {
            return true;
          }
        }
    ...
    function isAllowedModule(moduleName) {
      return allowedModulesSet.has(moduleName);
    }
    ...
    function checkFsModule(node) {
      ...
      if (node.callee.type === 'MemberExpression' &&
        ...
        if (FS_METHODS.has(method)) {
          ...
          const messageId = FS_FILE_METHODS.includes(method)
            ? 'unmockedFs'
            : 'mockFs';
          context.report({
            node,
    ...
    function checkDestructuredImports(node) {
      ...
      if (node.callee.name && FS_METHODS.has(node.callee.name)) {
