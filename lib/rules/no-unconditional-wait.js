...
const { isTestFile, isInMockContext, findEnclosingFunction, ensureAsyncFunction, addWaitForImport, getTestFramework } = require('../utils/helpers');
...
module.exports = {
  meta: {
    ...
    messages: {
      ...
      useWaitForPlaywright: 'Use Playwright auto-retrying assertions (expect(locator).toBeVisible()) instead of fixed delay.',
      useWaitForCypress: 'Use .should() with built-in retry or cy.intercept() + cy.wait(@alias) instead of fixed delay.',
        ...
        if (arg && arg.type === 'ArrowFunctionExpression') {
          ...
          if (body.type === 'CallExpression' &&
            ...
            if (setTimeoutFirstArg &&
              ...
              const ucwFramework = getTestFramework(context);
              const ucwMessageId = ucwFramework === 'playwright' ? 'useWaitForPlaywright'
                : ucwFramework === 'cypress' ? 'useWaitForCypress'
                  : 'useWaitFor';

              context.report({
                node,
                messageId: ucwMessageId,
                fix(fixer) {
