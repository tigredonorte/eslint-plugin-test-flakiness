...
const { isTestFile, isInHook, isInsideBrowserContext } = require('../utils/helpers');
  ...
  create(context) {
    ...
    return {
      AssignmentExpression(node) {
        // Mutations inside browser-context callbacks (page.evaluate/addInitScript/
        // evaluateHandle) run in the page, not Node — never flaky in the test
        // process. Guard at the visitor entry so EVERY sub-check inherits the
        // exemption (including checkGlobalVariableDeclaration, which would
        // otherwise still flag a bare `someGlobal = 5` browser global).
        if (isInsideBrowserContext(node)) return;
