...
module.exports = {
  ...
  create(context) {
    ...
    function checkGlobalAssignment(node) {
      ...
      // Check for nested assignments like window.location.href = {} or document.body.innerHTML
      // We need to check this BEFORE checking for direct identifier access
      if (object.type === 'MemberExpression') {
        const rootObject = getRootObject(object);
        if (rootObject && globalObjects.includes(rootObject.name)) {
          // Skip process.env assignments - they're handled specifically above
      ...
      // Now check for direct global object access (window.prop = value)
      ...
      // Don't flag process.env assignments here - they're handled separately above
