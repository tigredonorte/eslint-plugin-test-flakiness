...
const { isTestFile, findEnclosingFunction, ensureAsyncFunction, addWaitForImport, getTestFramework, getFilename } = require('../utils/helpers');
...
module.exports = {
  meta: {
    ...
    messages: {
      ...
      avoidFocusCheckPlaywright: 'Focus checks can be flaky. Use await expect(locator).toBeFocused() for auto-retry.',
      avoidFocusCheckCypress: 'Focus checks can be flaky. Use cy.focused().should() for built-in retry.',
      ...
      avoidActiveElementPlaywright: 'document.activeElement checks are timing-dependent. Use await expect(locator).toBeFocused() instead.',
  ...
  create(context) {
    ...
    const handledAssertionNodes = new Set(); // Track assertion nodes handled by combined focus+assertion fixer
    const framework = getTestFramework(context);

    function getFrameworkMessageId(baseId) {
      if (framework === 'playwright' && (baseId + 'Playwright') in module.exports.meta.messages) {
        return baseId + 'Playwright';
      }
      if (framework === 'cypress' && (baseId + 'Cypress') in module.exports.meta.messages) {
        return baseId + 'Cypress';
      }
      return baseId;
    }
    ...
    function reportOnce(node, messageId, fix) {
      const resolvedMessageId = getFrameworkMessageId(messageId);
    ...
    function checkFocusAssertion(node) {
      if (node.callee.type === 'MemberExpression') {
        ...
        if (method === 'toHaveFocus' ||
          ...
          // Check if this assertion was already handled by the combined focus+assertion fixer
          let exprStmt = node.parent;
          while (exprStmt && exprStmt.type !== 'ExpressionStatement') {
            exprStmt = exprStmt.parent;
          }
          const fix = (exprStmt && handledAssertionNodes.has(exprStmt)) ? undefined : createWaitForFix(node);

          reportOnce(node, 'avoidFocusCheck', fix);
        }
      ...
      if (node.type === 'MemberExpression' &&
        ...
        while (parent && parent.type !== 'Program') {
          if (parent.type === 'CallExpression' &&
            ...
            // Check if this assertion was already handled by the combined focus+assertion fixer
            let activeExprStmt = node.parent;
            while (activeExprStmt && activeExprStmt.type !== 'ExpressionStatement') {
              activeExprStmt = activeExprStmt.parent;
            }
            if (activeExprStmt && handledAssertionNodes.has(activeExprStmt)) {
              reportOnce(node, 'avoidActiveElement');
              break;
            }
      ...
      if (statementNode.type === 'ExpressionStatement') {
        ...
        if (block && (block.type === 'BlockStatement' || block.type === 'Program')) {
          ...
          if (nextStatement) {
            ...
            if (/expect.*(focus|activeElement)/i.test(nextSource) && nextStatement.type === 'ExpressionStatement') {
              // Track assertion so checkFocusAssertion/checkActiveElement skip their own fix
              handledAssertionNodes.add(nextStatement);

              // Combined fixer: wrap focus in act() AND assertion in waitFor() in one pass
              const focusCallText = sourceCode.getText(node);
              const multiStmt = hasMultipleStatementsOnLine(statementNode);
              reportOnce(node, 'useWaitForFocus', function(fixer) {
                ...
                const suffix = multiStmt ? ';' : '';
                const assertionExprText = sourceCode.getText(nextStatement.expression);

                return [
                  fixer.replaceText(statementNode, `await act(async () => { ${focusCallText} })${suffix}`),
                  fixer.replaceText(nextStatement, `await waitFor(() => ${assertionExprText})`),
                  ...asyncFixes,
                  ...(importFixes || [])
                ];
              });
        ...
        const standaloneFocusCall = sourceCode.getText(node);
        const standaloneMultiStmt = hasMultipleStatementsOnLine(statementNode);
        reportOnce(node, 'useWaitForFocus', function(fixer) {
          const funcNode = findEnclosingFunction(node);
          const asyncFixes = ensureAsyncFunction(fixer, funcNode);
          if (asyncFixes === null) return null;

          const suffix = standaloneMultiStmt ? ';' : '';
          return [fixer.replaceText(statementNode, `await act(async () => { ${standaloneFocusCall} })${suffix}`), ...asyncFixes];
        });
        return;
      }

      // Report focus calls in other contexts (like ReturnStatement in IIFE)
      const iifeFocusCall = sourceCode.getText(node);
      const iifeMultiStmt = statementNode.type === 'CallExpression' &&
        statementNode.parent && hasMultipleStatementsOnLine(statementNode.parent);
      reportOnce(node, 'useWaitForFocus', function(fixer) {
        // Use statementNode (the IIFE) not node (inside the IIFE) to avoid overlapping fixes
        const funcNode = findEnclosingFunction(statementNode);
        const asyncFixes = ensureAsyncFunction(fixer, funcNode);
        if (asyncFixes === null) return null;
        ...
        if (statementNode.type === 'CallExpression') {
          ...
          const suffix = iifeMultiStmt ? ';' : '';
          return [fixer.replaceText(statementNode, `await act(async () => { ${iifeFocusCall} })${suffix}`), ...asyncFixes];
        }
