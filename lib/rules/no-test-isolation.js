  ...
  create(context) {
    ...
    return {
      ...
      VariableDeclaration(node) {
        ...
        const isInSetupHook = isInHook(node, ['beforeEach', 'beforeAll']);
        ...
        node.declarations.forEach(declarator => {
          ...
          // Track variable declarations in setup hooks for initialization tracking
          if (isInSetupHook) {
      ...
      Identifier(node) {
        // Check for variable usage (not assignment) in tests
        const variableName = node.name;

        // Skip if this is part of an assignment or update expression (handled elsewhere)
        const parent = node.parent;
        if (parent.type === 'AssignmentExpression' && parent.left === node) return;
        if (parent.type === 'UpdateExpression' && parent.argument === node) return;
        if (parent.type === 'VariableDeclarator' && parent.id === node) return;
        if (parent.type === 'Property' && parent.key === node) return;
        if (parent.type === 'MemberExpression' && parent.property === node && !parent.computed) return;

        // Check if this variable is used in a test context
        if (isInTest(node) && describeVariables.has(variableName)) {
          const varInfo = describeVariables.get(variableName);

          // If variable has an initializer, it should be moved to setup
          if (varInfo.hasInitializer) {
            const key = `${variableName}-describe-usage-initInSetup`;
            if (!reportedViolations.has(key)) {
              reportedViolations.add(key);
              pendingReports.push({
                node: varInfo.node,
          ...
          // If variable has no initializer but is also not initialized in setup hooks, it's shared state
          else if (!hasSetupInitialization(variableName)) {
            const key = `${variableName}-describe-usage`;
            if (!reportedViolations.has(key)) {
              reportedViolations.add(key);
              pendingReports.push({
                node,
          ...
          // If variable has no initializer AND is initialized in setup hooks, this is correct - don't report
        }
      },
      ...
      CallExpression(node) {
        ...
        if (node.callee.type === 'MemberExpression' &&
          ...
          // Check for mutating method calls - always report these
          if ((moduleVariables.has(objectName) || describeVariables.has(objectName)) &&
              MUTATING_METHODS.includes(method)) {
            // For describe variables, check if properly initialized in setup
            if (describeVariables.has(objectName)) {
              const varInfo = describeVariables.get(objectName);
              if (varInfo.hasInitializer || !hasSetupInitialization(objectName)) {
            ...
            // For module variables, always report
            else if (moduleVariables.has(objectName)) {
              const key = `${objectName}-method-${method}`;
              if (!reportedViolations.has(key)) {
                reportedViolations.add(key);
                pendingReports.push({
