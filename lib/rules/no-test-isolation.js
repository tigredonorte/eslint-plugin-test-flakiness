...
module.exports = {
  ...
  create(context) {
    ...
    function isConstantDeclaration(node) {
      ...
      return node.declarations.every(declarator => {
        ...
        if (declarator.init.type === 'ObjectExpression') {
          return false; // Objects are always mutable - properties can be added, modified, or deleted
        }

        // Arrays are also always mutable
        if (declarator.init.type === 'ArrayExpression') {
          return false; // Arrays are always mutable - elements can be added, modified, or removed
        }
    ...
    function findCleanupForSetup(setupHook, _setupNode) {
      ...
      const hookPattern = new RegExp(`${cleanupHook}\\s*\\(`, 'g');
      ...
      return staticCleanupPatterns.some(pattern => {
      ...
      CallExpression(node) {
        ...
        if (node.callee.type === 'MemberExpression' &&
          ...
          if ((moduleVariables.has(objectName) || describeVariables.has(objectName)) &&
            ...
            if (meaningfulMethods.includes(method) || LOWERCASE_START_REGEX.test(method)) {
