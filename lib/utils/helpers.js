...
function addWaitForImport(fixer, context) {
  ...
  // Try to augment existing @testing-library import
  ...
  if (ast && ast.body) {
    for (const node of ast.body) {
      if (node.type === 'ImportDeclaration' &&
        ...
        // Augment this import with waitFor
        const lastSpecifier = node.specifiers[node.specifiers.length - 1];
        if (lastSpecifier) {
          return [fixer.insertTextAfter(lastSpecifier, ', waitFor')];
        }
      }
    }
  }

  // No @testing-library import to augment
  return [];
}
