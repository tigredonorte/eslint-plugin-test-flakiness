 ...
 * @returns {Array} Array of fixer operations (empty if already async or null)
 */
function ensureAsyncFunction(fixer, funcNode) {
  ...
  if (parent && (parent.type === 'MethodDefinition' || (parent.type === 'Property' && parent.method))) {
    // Getters, setters, and constructors cannot be async
    if (parent.kind === 'get' || parent.kind === 'set' || parent.kind === 'constructor') return [];
...
function addWaitForImport(fixer, context) {
  ...
  // Check if waitFor is already imported/available
  if (/\bimport\b[^;]*\bwaitFor\b/.test(text) ||
      /\bwaitFor\b[^;]*=\s*require/.test(text) ||
      /require\([^)]*\)\s*\.\s*waitFor\b/.test(text) ||
      /{\s*[^}]*\bwaitFor\b[^}]*}\s*=\s*require/.test(text)) {
    return [];
  }

  // Try to augment existing @testing-library import (only named imports)
  const ast = sourceCode.ast;
  if (ast && ast.body) {
    for (const node of ast.body) {
      if (node.type === 'ImportDeclaration' &&
          node.source && node.source.value &&
          node.source.value.startsWith('@testing-library/')) {
        // Only augment if there are named import specifiers (not namespace or default-only)
        ...
        // For namespace/default imports, add a separate named import on the next line
        return [fixer.insertTextAfter(node, '\nimport { waitFor } from \'' + node.source.value + '\';')];
      }
    }
  }

  // No @testing-library import found — add a new one at the top of the file
